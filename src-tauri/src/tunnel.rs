...
/// 建立一条隧道 (供 ssh.rs 连接成功后自动建立复用; 也作为 tunnel_start 命令的实现)。
pub async fn start_tunnel(
    ...
    // 登记到会话隧道注册表
    {
        ...
        guard.entry(session_id.to_string()).or_default().push(record);
    }

    crate::audit::log(crate::audit::AuditEntry {
        r#type: Some("tunnel.start".into()),
        session: Some(session_id.to_string()),
        target: Some(format!("127.0.0.1:{} -> {}:{}", local_port, remote_host, remote_port)),
        result: Some("success".into()),
        detail: if name.is_empty() { None } else { Some(format!("隧道: {}", name)) },
        ..Default::default()
    });
...
pub async fn tunnel_start(
    ...
    start_tunnel(&state, &session_id, cfg).await
}
...
pub fn tunnel_stop(session_id: String, tunnel_id: String, state: State<'_, AppState>) -> TunnelStopResult {
    ...
    crate::audit::log(crate::audit::AuditEntry {
        r#type: Some("tunnel.stop".into()),
        session: Some(session_id),
        target: Some(format!("{}:{}", record.local_port, record.remote_port)),
        result: Some("success".into()),
        detail: if record.name.is_empty() { None } else { Some(format!("隧道: {}", record.name)) },
        ..Default::default()
    });
