...
fn dpapi_encrypt(plain: &[u8]) -> Option<Vec<u8>> {
    ...
    unsafe {
        ...
        // TODO(verify): windows 0.58 中 dwFlags 参数类型为 CRYPTPROTECT_FLAGS,
        // 若编译报类型不匹配, 改传 0u32 或 CRYPTPROTECT_UI_FORBIDDEN.0。
        ...
        // 释放由 DPAPI 分配的缓冲区。
        // TODO(verify): windows 0.58 中 LocalFree 位于 Win32::System::Memory,
        //   参数类型为 HLOCAL (*mut c_void); 若编译报类型不匹配, 改回 as isize。
        if !output.pbData.is_null() {
            windows::Win32::System::Memory::LocalFree(output.pbData as *mut core::ffi::c_void);
        }
        Some(bytes)
    }
}
