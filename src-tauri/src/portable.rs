...
pub async fn config_export(
    ...
    let build = tauri::async_runtime::spawn_blocking(move || build_file_bytes(&pwd, &clean))
        .await
        .map_err(|_| "导出异常")?;
    ...
    // 写文件
    let write_res = tauri::async_runtime::spawn_blocking(move || std::fs::write(&path, &bytes))
        .await
        .map_err(|_| "导出异常")?;
    match write_res {
...
pub async fn config_import(
    ...
    // 读文件（大小上限 5MB）
    let read_res = tauri::async_runtime::spawn_blocking(move || -> Result<Vec<u8>, String> {
        let meta = std::fs::metadata(&path).map_err(|_| "读取文件失败")?;
        if meta.len() > MAX_FILE_BYTES as u64 {
            return Err("文件格式无效".to_string());
        }
        std::fs::read(&path).map_err(|_| "读取文件失败")
    })
    .await
    .map_err(|_| "导入异常")?;
    ...
    let pwd = password.clone();
    let decrypt = tauri::async_runtime::spawn_blocking(move || parse_file_bytes(&pwd, &data))
        .await
        .map_err(|_| "导入异常")?;
