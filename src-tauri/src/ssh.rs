...
pub struct ConnectConfig {
    ...
    #[serde(default)]
    pub host_key_verify: Option<String>,
}
...
fn load_key_pair(
    ...
    russh::keys::load_secret_key(path, passphrase).map_err(|_| "无法读取私钥")
}
...
#[tauri::command]
pub async fn ssh_disconnect(state: State<'_, AppState>, session_id: String) -> crate::CmdOk {
    let handle = {
        let sessions = state.sessions.lock().unwrap();
        sessions.get(&session_id).cloned()
    };
