...
/// 注: 命令保持私有可见性 (Tauri 宏生成 `__cmd__*` 宏 + `use` 重导入, 带任何可见性修饰
/// 都会在 crate 根模块产生 E0255 宏命名空间冲突; 私有 fn 不生成 #[macro_export])。
#[tauri::command]
async fn dialog_select_key(app: AppHandle) -> DialogResult {
...
/// 修复: 接受 defaultName 参数并作为保存对话框默认文件名 (原实现忽略了前端传入的 defaultName)。
#[tauri::command]
async fn dialog_select_save_path(
    app: AppHandle,
    ...
    if let Some(name) = default_name {
        let name = name.trim();
        if !name.is_empty() {
            // TODO(verify): tauri-plugin-dialog 2 FileDialogBuilder::set_file_name 存在性
...
#[tauri::command]
fn store_load() -> Vec<serde_json::Value> {
    store::load_redacted()
}

/// storeSave: fail-closed 保存连接列表 (留空沿用磁盘旧凭据)。
#[tauri::command]
fn store_save(list: Vec<serde_json::Value>) -> CmdOk {
...
/// settingsSave: 保存全局设置。
#[tauri::command]
fn settings_save(settings: serde_json::Value) -> CmdOk {
...
#[tauri::command]
fn audit_log(entry: audit::AuditEntry) -> CmdOk {
    audit::log(entry);
    CmdOk::success()
}
...
#[tauri::command]
fn audit_query(filters: audit::AuditQueryFilters) -> audit::AuditQueryResult {
    audit::query(&filters)
}
...
#[tauri::command]
fn open_external(app: AppHandle, url: String) -> CmdOk {
    let lower = url.to_ascii_lowercase();
    if !(lower.starts_with("http://") || lower.starts_with("https://")) {
        return CmdOk::failure("仅支持打开 http/https 链接");
    }
    // tauri-plugin-shell 2: ShellExt::open 参数为 impl Into<String> (不是路径)。
    ...
    tauri::Builder::default()
        ...
        .register_uri_scheme_protocol("nimbus-preview", |_ctx, req| {
            ...
            match preview_doc::read_registered_tmp(&filename, "preview") {
                Some((bytes, mime)) => http_ok(bytes, mime),
                None => http_not_found(),
            }
        })
        // nimbus-doc: 文档查看器临时文件 (nimbus-doc- 前缀)
        .register_uri_scheme_protocol("nimbus-doc", |_ctx, req| {
            let url = req.uri().to_string();
            let filename = match preview_doc::parse_protocol_filename(&url) {
                Some(f) => f,
                None => return http_not_found(),
