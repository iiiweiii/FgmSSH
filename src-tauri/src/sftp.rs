    ...
    if attrs.is_dir() {
        if let Ok(entries) = sftp.read_dir(path).await {
            for entry in entries {
                ...
                remove_recursive(sftp, &child, depth + 1).await?;
            }
        }
        sftp.remove_dir(path).await.map_err(|_| "删除失败")
    } else {
        sftp.remove_file(path).await.map_err(|_| "删除失败")
    }
}
...
/// 搜索关键字白名单：仅保留 [A-Za-z0-9_.-]，其余（空格/引号/分号/$ 等 shell 元字符）一律剔除。
...
async fn collect_folder_files(
    ...
    for entry in entries {
        ...
        if is_dir {
            collect_folder_files(sftp, &child_remote, &child_zip, out, depth + 1).await?;
        }
    }
    Ok(())
}
