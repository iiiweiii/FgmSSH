...
async function fetchMonitorData({ exec, identity }) {
  ...
  const data = {
    ...
    info: {
      hostname: results.hostname ? results.hostname.trim() || null : null,
      os: results.os ? parseOsRelease(results.os) : null,
      date: results.date ? results.date.trim() || null : null,
    },
  ...
  if (results.load && !data.load) errors.load = '负载输出无法解析';
  if (results.memory && !data.memory) errors.memory = '内存输出无法解析';
  if (results.cpu && !data.cpu) errors.cpu = 'CPU 输出无法解析';
  if (results.os && !data.info.os) errors.os = '系统信息无法解析';
  if (results.gpu && !data.gpu) errors.gpu = 'GPU 输出无法解析';
