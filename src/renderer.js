...
async function refreshMonitor(silent) {
  ...
  // 修复: 后端 ssh_monitor_fetch 只返回 raw 原始命令输出, 结构化字段 (info/load/memory/
  // disks/cpu/gpu) 需由前端 health-parser 解析填充 (原实现缺该步, 监控面板全部"无法获取")。
  if (res && res.raw && typeof res.raw === 'object' && window.HealthParser) {
    try {
      const results = {};
      Object.keys(res.raw).forEach((section) => {
        const item = res.raw[section];
        if (item && typeof item.stdout === 'string') results[section] = item.stdout;
      });
      const identity = s.config ? `${s.config.username || ''}@${s.config.host || ''}` : '';
      const parsed = window.HealthParser.parseMonitorResults(results, identity, res.errors || {});
      res = Object.assign({}, res, parsed);
    } catch (e) {
      console.warn('[monitor] 监控数据解析失败:', e);
    }
  }
