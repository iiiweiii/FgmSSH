...
> **当前状态**：Tauri v2 版源码已交付，**待本机构建验证**。代码中标注了 `// TODO(verify)` 复核点（russh / windows crate 等 API 差异），详见 [README-BUILD.md](README-BUILD.md)。
...
| 版本 | 技术栈 | 安装包体积 |
| --- | --- | --- |
| 旧版（tag `v1.1.0-electron`） | Electron + 内置 Node.js 运行时 | ~80MB |
| 新版（master，Tauri v2） | Tauri v2 + Rust + 系统 WebView2 | **预计 8–15MB**（待构建验证） |
